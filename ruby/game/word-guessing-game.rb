  ...
  def update_word(guesses_made)
    @guesses_made.each do |guess, nature|
      ...
      if (nature == true) && (is_repeated_in_word(guess) == true)
        ...
        array_repeated.each { |index| number_slots_array[index] = guess}
        #method will go through the array_repeated, and insert the guess in each part of number_slots_array that match
